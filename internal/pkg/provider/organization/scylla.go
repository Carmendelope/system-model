...
func (sp *ScyllaOrganizationProvider) Remove(organizationID string) derrors.Error {
	...
	stmt, _ := qb.Delete(organizationTable).Where(qb.Eq("organization_id")).ToCql()
	cqlErr := sp.Session.Query(stmt, organizationID).Exec()

	if cqlErr != nil {
		return derrors.AsError(cqlErr, "cannot delete organization")
	}
